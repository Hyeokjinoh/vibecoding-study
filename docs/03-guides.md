...
- [ ] 가이드와 센서의 차이를 시점(전/후)과 성격(피드포워드/피드백)으로 설명할 수 있나요
- [ ] 여러 `CLAUDE.md` 가 덮어쓰기가 아니라 이어붙임으로 로드된다는 것을 알고, `/context` 로 확인할 수 있나요
- [ ] `@경로` import 가 컨텍스트를 아껴 주지 않는 이유를 말할 수 있고, 최대 홉 수를 아나요
- [ ] **로드 시점**(항상 / 경로별 / 호출 시)으로 규칙을 어디에 둘지 정할 수 있고, `CLAUDE.md` 권장 상한을 아나요
- [ ] 스킬의 점진적 공개를 설명할 수 있고, 문서로 적는 것과 `tools`·훅으로 강제하는 것의 차이를 구분할 수 있나요
