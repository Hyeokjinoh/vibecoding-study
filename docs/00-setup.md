...
- [ ] 내 플랜의 세션이 어떤 모드로 시작하는지 말할 수 있습니다. `Shift+Tab` 으로 `default` / `acceptEdits` / `plan` 을 순환시키며 상태 표시줄 문구도 구분할 수 있나요
- [ ] 설정 파일 5단계 중 어느 것이 더 세고, 권한이 **deny → ask → allow** 순으로 평가된다는 것을 설명할 수 있나요
- [ ] 이 저장소의 `.claude/settings.json` 에서 `git push` 가 `ask` 인 이유와 `curl` 이 `deny` 인 이유를 각각 한 문장으로 말할 수 있나요
- [ ] `/init`, `/memory`, `/context` 가 각각 무엇을 하는지 구분할 수 있나요
- [ ] 시드 테스트가 실패하는 게 왜 정상인지, 그 출력이 왜 "센서"인지 설명할 수 있나요
- [ ] `python -m pytest --version` 이 정상 출력되고, **"테스트가 실패한 것"과 "검사가 실행되지 않은 것"** 의 차이를 말할 수 있나요
